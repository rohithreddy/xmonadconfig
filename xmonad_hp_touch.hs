...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    --, ((modMask,               xK_Up    ), raiseMaybe
    --   (spawn "caja ~") (className =? "Caja" <&&> title=?"max"))
    ...
    , ((modMask,               xK_f     ), runOrRaise
        "/home/max/scriptz/fix_ff_touch.sh" (className =? "Firefox"))
     -- browser
    , ((modMask,               xK_c     ), runOrRaise
        "google-chrome"(className =? "Google-chrome"))
...
-- layouts 
myLayout = tiled ||| Circle ||| Grid ||| mosaic 2 [3,2]  |||  simpleFloat' shrinkText myTab ||| ThreeColMid nmaster (delta) (ratio)  |||   spiral (6/7)   |||  layoutHints (tabbed shrinkText myTab) 
    where
...
myDeco = def
    ...
    , urgentColor         = "#0498C0"
    , activeBorderColor   = "#0498C0"
    , inactiveBorderColor = "#222222"
    , urgentBorderColor   = "#0498C0"
    , activeTextColor     = "#0498C0"
    , inactiveTextColor   = "#222222"
    , urgentTextColor     = "#0498C0"
    , decoHeight          = 10 }
